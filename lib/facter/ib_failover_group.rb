...
# Get compartment id and tier of current instance
begin
...
# Get environment tag of current instance
begin
  tag = compute_client.get_instance(instance_id).data
rescue
  debug_msg("Unable to find tag " + tags['environment'] + " assigned to instance")
  exit 1
end
  ...
  if inst.defined_tags[tags['namespace']][tags['grouping']] == 'ib' then
    compute_client.list_vnic_attachments(compartmentid, instance_id: inst.id).each { |vnic_attachments|
      vnic_attachments.data.each { |attachment|
        ...
        ib_failover_group.push(vnic.private_ip)
      }
    }
  end
}
